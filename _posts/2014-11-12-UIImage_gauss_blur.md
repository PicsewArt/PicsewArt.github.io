...
- (UIImage *)gaussBlurWithLevel:(CGFloat)blurLevel {
    ...
    UIImage *decode = [UIImage decode:self];
    NSData *imageData = UIImageJPEGRepresentation((decode ? decode : self), 1);
    if (imageData == nil) {
        return self;
    }
    ...
    if (error) {
        MXLog(@"error from convolution %ld", error);
    }
